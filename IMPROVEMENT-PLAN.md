...
4. **Scientific tracking experiments (deferred from release).** The `experiment/scientific-tracking` branch contains opt-in seeded point selection and alternative exclusion-mask policies while retaining released modes as defaults. Compare representative matches and turnover against independent annotations before deciding whether either experiment belongs in a future release.
