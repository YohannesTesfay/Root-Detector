...
Keep the tested core branch, the larger local feature branch, and packaging follow-up separate:

1. **Validated core package — complete.** The full Windows artifact was qualified through the broad `4b64890` matrix and the focused `e9b86bb` training retest. Preserve both reports as release evidence.
2. **Merge the narrow follow-up.** PR #3 is clean and its accepted head is `e9b86bb`. Mark it ready for review, merge it into `fix/core-automation`, and do not delete the core branch until the next step is complete.
3. **Merge the validated core.** Open or update the core PR from `fix/core-automation` into the fork's `main`, verify that it contains PR #3, and merge only after its checks are green.
4. **Finish post-core stabilization.** The narrow follow-up upgrades to Node-24-native action majors, publishes one clearly named full user ZIP, removes the legacy delta ZIP from user artifacts, and improves the controls involved in acceptance. Complete contextual help and prevent Windows sleep during active processing/training while restoring the prior power policy afterward.
5. **Resume the larger plan.** Bring the updated fork `main` into `feature/reliability-security-hardening`, resolve changes deliberately, rerun fast/model/browser/Windows acceptance, and then continue Phase 0 below.
...
| T-002 | P0 — implemented in narrow follow-up; carry forward on branch sync | The browser previously sent `learning_rate` while the backend read `lr`, silently substituting `0.001`. | Treat `learning_rate` as the canonical public field, accept legacy `lr`, reject conflicting/invalid values, report the effective value, and prove the optimizer receives it exactly. Preserve this contract when syncing `feature/reliability-security-hardening`. |
