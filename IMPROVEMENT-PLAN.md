...
| T-012 | P1 — open; scientific repeatability | The released matcher samples root points through an unseeded `numpy.random.permutation` in `backend/tracking_matcher.py`. On the same packaged build, repeating one identical Eldena pair changed matched points from 3,863 to 3,890 and same/decay/growth pixel counts. All nine detection statistics were identical across builds. | Preserve the existing algorithm as version 1; design a versioned deterministic mode with a local RNG seeded from stable input/model identities, record seed and sampling version in every result, and add same-process/restart/CPU/GPU repeatability tests. Compare the new mode with version 1 across representative pairs and obtain ecological approval before changing the default or interpreting quantitative turnover. |
