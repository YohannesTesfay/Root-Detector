...
**Merged-fork checkpoint:** PRs #4 and #5 are in fork `main` at `9b8de6c`; its exact portable build completed the nine-image/six-pair Eldena GPU replay. Tracking mechanics pass, but an unseeded sampling step makes turnover counts vary between repeat runs (T-012). Treat those numbers as exploratory until repeatability is addressed.
...
2. **Fork integration complete.** PR #4 was merged into fork `main`, PR #5 was retargeted, reviewed as the RC2-only delta, and merged. The resulting `9b8de6c` tree is identical to the qualified RC2 branch tree.
3. **Merged-main Windows mechanics accepted.** Actions run `35354845509` built the exact `9b8de6c` head. A fresh `ExPlEco_ML_Desk` extraction completed nine CUDA detections, six genuine-date same-level pairs, tracking export, and diagnostics. The browser reviewed-label/training/cancel tests and repeated 86-image resource gate were qualified on earlier packages with the same source; details and hashes are in `WINDOWS-GPU-ACCEPTANCE.md`.
4. **Supervised use only; quantitative repeatability open.** Do not block supervised data collection on ecological interpretation, but label tracking outputs as requiring review and retain every run's input/model/settings provenance. A repeat of one pair changed tracking counts because the released matcher samples points randomly without a fixed seed (T-012). Do not treat single-run turnover counts as deterministic, use self-generated masks as ground truth, or release a newly trained model as scientifically validated.
...
| T-012 | P1 — opt-in implementation; packaged/scientific validation pending | The released matcher samples root points through an unseeded `numpy.random.permutation` in `backend/tracking_matcher.py`. On the same packaged build, repeating one identical Eldena pair changed matched points from 3,863 to 3,890 and same/decay/growth pixel counts. All nine detection statistics were identical across builds. | Legacy version 1 remains the default. Opt-in version 2 uses a local seeded RNG derived from ordered input, segmentation, and model hashes; pair JSON/ZIP record the seed and version. Run same-process/restart/CPU/GPU repeatability tests on packaged builds, compare v2 with v1 across representative pairs, and obtain ecological approval before changing the default or interpreting quantitative turnover. |
