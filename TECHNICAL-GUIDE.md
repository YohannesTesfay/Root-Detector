...
`test-fast` covers state transitions, retry, cancellation, training jobs, tracking sampling/provenance, request validation, download integrity, cache invalidation, and exports. `test-smoke` uses released models to check detection/tracking/export, legacy matcher equivalence, and a repeated seeded tracking pair.
